...
use glam::{Vec2, vec2};
use scal_core::{CodeAnimationStyle, CodeHighlightAction, Color, Ease, Syntax, Theme};
...
pub fn language(syntax: Syntax) -> tree_sitter::Language {
    match syntax {
...
fn highlight_config(syntax: &Syntax) -> Result<HighlightConfiguration> {
    match syntax {
...
impl Code {
    pub fn instantiate(&self) -> AnimOperation {
        AnimOperation::Instantiate(DynAnimObj(Box::new(self.clone())), None)
    }
    ...
    pub fn modify_line(
        ...
        AnimOperation::CodeModifyLine(self.id, line, new_text, duration, anim_curve, style, None)
    }
    ...
    pub fn remove_lines(
        ...
        AnimOperation::CodeRemoveLines(self.id, lines, duration, anim_curve, style, None)
    }
    pub fn new(
        text: String,
        syntax: Syntax,
