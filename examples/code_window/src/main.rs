...
fn fib(n: u32) -> u32 {
    ...
    match n {
        ...
        _ => fib(n - 1) + fib(n - 2),
            ...
            "#, 4,
            AnimationCurve::Linear, 5.0, CodeAnimationStyle::TypeWriter,
        ),
        wait(1.0),
        pointer.transform().position_to_object(
            &cw.close_btn, Vec2::ONE * 15., 1.0, AnimationCurve::EaseOutBack,
        ),
        cw.close_btn.transform().scale_to(Vec2::ONE * 0.85, 0.1, AnimationCurve::EaseOutCubic),
        cw.close_btn.transform().scale_to(Vec2::ONE, 0.15, AnimationCurve::EaseOutCubic),
        all(vec![
