...
use scal::{
    anim_object::text::{Align, code::{Syntax, theme::Theme}},
    prelude::*,
    projection::Camera,
};
...
async fn main() -> Result<()> {
    ...
    scal::run_loop(&handle, encoding_settings, rendering_settings, vec![
        ...
        wait(1.0),
        all(vec![
            triangle.transform().position_to(vec2(350., 800.), 1.0, AnimationCurve::EaseOutBack),
            rect.transform().position_to(vec2(0.5, 0.5), 1.0, AnimationCurve::EaseOutBack),
        ]),
        (rect.transform().position_to(CANVAS_SIZE / 2., 1.0, AnimationCurve::EaseInOutBack)),
    ]).await?;
    info!("Hello, world!");
    Ok(())
}
