...
impl TimeTextAtlas {
    ...
    fn measure_text(&mut self, text: &str, font_size: f32) -> (f32, f32) {
        ...
        let attrs = Attrs::new();
        buffer.set_text(text, &attrs, Shaping::Advanced, Some(cosmic_text::Align::Left));
        buffer.shape_until_scroll(&mut self.font_system, false);

        let (width, total_height) = buffer
            .layout_runs()
            .fold((0.0f32, 0.0f32), |(w, h), run| {
    ...
    fn rasterize_text(&mut self, text: &str, font_size: f32) -> (Vec<u8>, u32, u32) {
        let metrics = Metrics::new(font_size, font_size * 1.2);
        let mut buffer = cosmic_text::Buffer::new(&mut self.font_system, metrics);
        buffer.set_text(text, &Attrs::new(), Shaping::Advanced, Some(cosmic_text::Align::Left));
        buffer.shape_until_scroll(&mut self.font_system, false);
    ...
    fn rebuild(&mut self, device: &wgpu::Device, queue: &wgpu::Queue, time_labels: &[(f32, String, f32)]) {
        self.labels.clear();
        self.dirty = true;
...
impl PreviewRenderer {
    ...
    fn build_ui_geometry(&mut self) {
        ...
        // Time tick marks + labels
        {
            ...
            if interval > 0.0 {
                ...
                let w_ = w;
                let x_positions: Vec<f32> = self.time_text_atlas.labels.iter()
        ...
        if self.selected_op_info.is_some() {
            ...
            append_rect(&mut vertices, &mut indices, 0.0, info_bar_top, w, info_bar_bot, bg, NO_UV);
            let indicator_color = [0.2, 0.9, 0.7, 1.0];
            append_rect(&mut vertices, &mut indices, 2.0, info_bar_top + 2.0, 4.0, info_bar_bot - 2.0, indicator_color, NO_UV);
        }
    ...
    pub fn advance_render_present(&mut self) -> Result<bool> {
        ...
        let viewport = self.video_viewport(
...
fn append_rect(
    ...
    vertices.push(UIVertex { position: [x0, y0], color, uv });
    vertices.push(UIVertex { position: [x1, y0], color, uv });
    vertices.push(UIVertex { position: [x1, y1], color, uv });
    vertices.push(UIVertex { position: [x0, y1], color, uv });
    indices.push(base);
    indices.push(base + 1);
    indices.push(base + 2);
    indices.push(base);
...
fn append_rect_textured(
    ...
    let base = vertices.len() as u32;
    vertices.push(UIVertex { position: [x0, y0], color, uv: [uv_min[0], uv_min[1]] });
    vertices.push(UIVertex { position: [x1, y0], color, uv: [uv_max[0], uv_min[1]] });
    vertices.push(UIVertex { position: [x1, y1], color, uv: [uv_max[0], uv_max[1]] });
    vertices.push(UIVertex { position: [x0, y1], color, uv: [uv_min[0], uv_max[1]] });
    indices.push(base);
    indices.push(base + 1);
    indices.push(base + 2);
    indices.push(base);
